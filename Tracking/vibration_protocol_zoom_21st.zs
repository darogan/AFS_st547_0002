...
DEFINE PRETRIAL_TIME  180 
DEFINE TWO_MIN_PRETRIAL_TIME 120
DEFINE TIME_BIN 1
DEFINE REST_TIME_BIN 1
DEFINE NUM_SEQUENCE 1
DEFINE NUM_VIB_SEQUENCE 1
DEFINE NUM_REST 180
DEFINE NUM_VIB 10
...
ACTION MAIN    		
    ...
    #INVOKE(LIGHT,1) #select if you want to run vibration trials with lights on or off 
    INVOKE(DARK,1)
	...
	VIDEO(VIDEO_LENGTH,"Zeb_vibrations")  #realtime video 
    
    SET(VIDEO_STREAM,1)
   	VIDEO(VIDEO_LENGTH,"Zeb_vibrations_1fps") #timelapse at 1 frame per sec  
    VIDEOSKIP(29)
    
	SET(VIDEO_STREAM,2)
   	VIDEO(VIDEO_LENGTH,"Zeb_vibrations_10fps")  #timelapse 10secs per frame 
    VIDEOSKIP(299)
	
    SET(LOG_STREAM,1)
  	SET(LOG_PERFRAME,ON)
    
	INVOKE(SEQUENCE,NUM_SEQUENCE) #1 
...
ACTION SEQUENCE
    ...
    INVOKE(REST,PRETRIAL_TIME)
    PICTURE("picture_rest")
    INVOKE(REST,PRETRIAL_TIME)
    PICTURE("picture_rest")
    INVOKE(REST,PRETRIAL_TIME)
    PICTURE("picture_rest")
    ...
    PICTURE("picture_rest")
    INVOKE(REST,PRETRIAL_TIME)
    PICTURE("picture_pre_rest")
    INVOKE(REST,TWO_MIN_PRETRIAL_TIME)
    
  	LOAD(X_OVERLAY_TEXT,"1,2:Habituation     ") 
    
    INVOKE(HABITUATION,NUM_VIB)
    PICTURE("habituation_traces")
    
   	INVOKE(REST,NUM_REST) 
    PICTURE("picture_post_rest")
    INVOKE(REST,NUM_REST)
...
ACTION REST         
	...
	LOGCREATE("RUNTIME|TEMPERATURE1|TEXT:REST|COUNTER1")
...
#half second before motorcommand
#   LOGDATA(DATA_SNAPSHOT,"begin") 
   	LOAD(X_OVERLAY_TEXT,"1,2:Habituation     ")   
#	LOAD(X_OVERLAY_TEXT,"1,26:Pre_Vibration  ")
...
#	LOGCREATE("RUNTIME|TEMPERATURE1|TEXT:PRE_VIBRATION|COUNTER1")
#	LOGAPPEND("ARENA_DISTANCES*|ZONE_DISTANCES:A1-24 Z1,2")
#  	LOGRUN()
    
# vibration set at 0.14sec, 476Hz  
 	SET(COUNTER1,COUNTER_INC) 
    
	LOGDATA(DATA_SNAPSHOT,"begin") 
    
  	LOAD(X_OVERLAY_TEXT,"1,26:Vibration      ") 
	MOTORCOMMAND("U0 D1000 M1 M-1 M1 M-1 M1 M-1 M1 M-1 M1 M-1 M1 M-1 M1 M-1 M1 M-1") 
  	WAIT(TIME_BIN)   #U0 D2000 

	LOGDATA(DATA_SNAPSHOT,"end") 
	LOGDATA(DATA_SELECT,"begin")
	LOGDATA(DATA_DELTA,"end")

	LOGCREATE("RUNTIME|TEMPERATURE1|TEXT:VIBRATION|COUNTER1")
	LOGAPPEND("ARENA_DISTANCES*|ZONE_DISTANCES:A1-24 Z1,2")
  	LOGRUN()
    
#1 second following motorcommand    
  	LOGDATA(DATA_SNAPSHOT,"begin") 
    LOAD(X_OVERLAY_TEXT,"1,26:Post_Vibration    ")
	...
	LOGCREATE("RUNTIME|TEMPERATURE1|TEXT:POST_VIBRATION|COUNTER1")
	LOGAPPEND("ARENA_DISTANCES*|ZONE_DISTANCES:A1-24 Z1,2")
  	LOGRUN()
